...
## Function: Plot and describe all expectations
...
Data$Mean_Change_Squared <- Data$Mean_Change %>% raise_to_power(2)

Data$MeanExpectedFuture_Variance_Minus <- (Data$MeanExpectedFuture - Data$Variance)
Data$MeanExpectedFuture_Variance_Plus <- (Data$MeanExpectedFuture + Data$Variance)

# ***************************************************
# Section 2B: Renaming for ease ####
# ***************************************************


Data$Current <- Data$MeanExpectedCurrent
Data$Future <- Data$MeanExpectedFuture
Data$Future_Neg <- Data$MeanExpectedFuture_Variance_Minus
Data$Future_Pos <- Data$MeanExpectedFuture_Variance_Plus
Data$Best <- Data$MeanExpectedBest
Data$Worst <- Data$MeanExpectedWorst
Data$Change <- Data$Mean_Change
...
Labels_Y <-
  c(
    paste0("Expected current\nharm\n", Data$Current %>% mean() %>% round(2), " (", Data$Current %>% sd() %>% round(2), ")"),
    paste0("Expected future\nharm\n", Data$Future %>% mean() %>% round(2), " (", Data$Future %>% sd() %>% round(2), ")"),
    paste0("Worst case\nscenario\n", Data$Worst %>% mean() %>% round(2), " (", Data$Worst %>% sd() %>% round(2), ")"),
    paste0("Best case\nscenario\n", Data$Best %>% mean() %>% round(2), " (", Data$Best %>% sd() %>% round(2), ")"),
    paste0("Future expectation\n + variance\n", Data$Future_Pos %>% mean() %>% round(2), " (", Data$Future_Pos %>% sd() %>% round(2), ")"),
    paste0("Future expectation\n - variance\n", "0.30", " (", Data$Future_Neg %>% sd() %>% round(2), ")")
  ) %>% rev()
...
ggsave(
  FigureX_Expectations,
  device = "png",
  filename = here("CVOutput", "FigureX_Expectations.png"),
  width = 25,
  height = 15,
  units = "cm",
  dpi = 500
