...
# Comment:
## - Outputs both table 1 and a figure
...
# 1. Basic descriptive statistics for both variables
descriptives <- Data %>%
  summarise(
    current_mean = mean(MeanExpectedCurrent, na.rm = TRUE),
    current_sd = sd(MeanExpectedCurrent, na.rm = TRUE),
    future_mean = mean(MeanExpectedFuture, na.rm = TRUE),
    future_sd = sd(MeanExpectedFuture, na.rm = TRUE)
  )
...
## Note, now Table 1 (19/01)
Table_B3 <- cbind(
  Data %>% dplyr::filter(Uncertainty == 0) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 2, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 1) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 2, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 3) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 2, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 5) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 2, affix_sign = TRUE)
)


cbind(
  Data %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 1, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 0) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 1, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 1) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 1, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 3) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 1, affix_sign = TRUE),
  Data %>% dplyr::filter(Uncertainty == 5) %>% tabyl(Bid, CV) %>% adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 1, affix_sign = TRUE)
)
