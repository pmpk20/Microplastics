...
Figure6 <- Expectations %>% ggplot(aes(x = value, y = Var2, group=Var2,fill = Var2)) + 
  stat_halfeye()+
  ggtitle("Expected Future Harm Of Microplastics") +
  scale_y_discrete(
    name = "Questions.") +
  scale_x_continuous(name = "Harm on a scale (Much better) to (Much worse).",
                     limits = c(-5, 10),
                     breaks = seq(-5, 10, 1)) + 
  scale_fill_manual(name="Question",
                    values=RColorBrewer::brewer.pal(9,"Blues")[c(5,7,9)],
                    guide=guide_legend(reverse = TRUE))+
...
ggsave(Figure6,device = "jpeg",
       filename = "Figure6_DensityPlot.jpeg",
       width=20,height=15,units = "cm",dpi=1000)
...
PlotOfAttitudes <- Attitudes %>% ggplot(aes(x = value, y = variable, group=variable,fill = variable)) + 
  ...
  scale_fill_manual(name="Item:",
                    values=RColorBrewer::brewer.pal(9,"Blues")[c(3,4,5,6,7,8,9)],
                    label=LegendLabels,
                    guide=guide_legend(reverse = TRUE))+
  coord_cartesian()+theme_bw()+
  theme(legend.background=element_blank(),
...
ggsave(PlotOfAttitudes,device = "jpeg",
       filename = "PlotOfAttitudes_2022_07_26.jpeg",
       width=20,height=15,units = "cm",dpi=1000)
