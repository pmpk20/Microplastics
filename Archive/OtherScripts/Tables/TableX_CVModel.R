...
TableX_Top <- bind_cols(
  "Variable" = summary(Model)$glm.summary$coefficients %>% rownames(),
  "Estimates" = summary(Model)$glm.summary$coefficients[, 1] %>% round(3),
  "SD" = summary(Model)$glm.summary$coefficients[, 2] %>% round(3),
  "P" = summary(Model)$glm.summary$coefficients[, 4] %>% AddStars()
)


TableX_Bottom <-  cbind(rbind(
  cbind("AIC", Model %>% AIC() %>% round(3)),
  cbind("Log-Likelihood", Model %>% logLik() %>% c() %>% round(3)
  ),
  cbind("Mean WTP", summary(Model)$medianWTP %>% round(3)
  )
),

rbind(
  cbind("BIC", Model %>% BIC() %>% round(3)),
  cbind("Adjusted R2", summary(Model)$adjpsdR2 %>% as.numeric() %>% round(3)
  ),
  cbind("Median WTP", summary(Model)$meanWTP %>% round(3)
  )
)) %>% data.frame()


## Not correct but allows us to merge
colnames(TableX_Bottom) <- colnames(TableX_Top)
...
TableX %>% fwrite(sep = ",",
                  here("Tables","TableX_CVModel.txt"))
