...
## Summarise expectations of current harms
Part1 <-  paste0(
  "Mean beliefs about how harmful microplastics currently are to human health were ",
  Data$MeanExpectedCurrent %>% mean() %>% round(2) %>% sprintf("%.2f", .),
  "/5 (SD: ",
  Data$MeanExpectedCurrent %>% sd() %>% round(2) %>% sprintf("%.2f", .),
  ")"
)


## Expected future is defined against the current value
Data$MeanExpectedFutureActual <- Data$MeanExpectedCurrent + Data$MeanExpectedFuture


## Future expectations
Part2 <-  paste0(
  "while the mean expected belief about the effect of microplastics on human health in ten years’ time without any changes was ",
  Data$MeanExpectedFutureActual %>% mean() %>% round(2) %>% sprintf("%.2f", .),
  " (SD: ",
  Data$MeanExpectedFutureActual %>% sd() %>% round(2) %>% sprintf("%.2f", .),
  ")"
)
