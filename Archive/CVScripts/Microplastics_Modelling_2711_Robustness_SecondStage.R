...
Model_C1_Output %>% 
  data.frame() %>% 
  fwrite(sep = ",",
         here("Tables", "Table_Model_C1_Output.txt"))
...
Model_C2_Diagnostics <- cbind(
  "Variable" = Model_C2$fit_statistics %>% names(),
  "Estimate" = Model_C2$fit_statistics
) %>% data.frame()


Model_C2_Output <- rbind(
...
Model_C2_Output %>% 
  data.frame()  %>% 
  fwrite(sep = ",",
         here("Tables", "Table_Model_C2_Output.txt"))
...
Model_C3_Diagnostics <- cbind(
  "Variable" = Model_C3$fit_statistics %>% names(),
  "Estimate" = Model_C3$fit_statistics
) %>% data.frame()


Model_C3_Output <- rbind(
  Model_C3$coefficients %>% 
    ModelOutput(Identifier = 1) %>% 
    dplyr::select(-Model),
  Model_C3_Diagnostics
) 

Model_C3_Output %>% write.csv(quote = FALSE, row.names = FALSE)
