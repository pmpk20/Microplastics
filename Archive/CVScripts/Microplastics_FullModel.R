...
## Last change: 16/11/23
# Comment:
## - Do not use this until translated sbchoice to glm()
...
# ****************************
# Section 2: All covariates Model ####
# *****************************


Data$MeanChange <- (Data$MeanExpectedFuture + Data$MeanExpectedCurrent)
...
# *****************************
# Section 3: All covariates Model ####
# *****************************

## Specify model
SBDC_Model_All <- sbchoice(
  CV ~ AgeDummy +
    Gender +
    EthnicityDummy +
    MeanExpectedFuture + 
    ...
    Q17_PandemicEnvironment + 
    Q17_PandemicMicroplastics + 
    MeanChange +
    Q16_Comparison +
    Coronavirus +
    IncomeDummy +
...
summary(SBDC_Model_All)


# *****************************
# Section 4: Fewer covariates Model ####
# *****************************


## Testing mean current or mean future
SBDC_Model_Newer_Current <- sbchoice(
  CV ~ AgeDummy +
    Gender +
    EthnicityDummy +
    MeanExpectedCurrent + 
...
SBDC_Model_Newer_Current %>% summary()
...
SBDC_Model_Newer_Future %>% summary()



## Testing mean current or mean future
SBDC_Model_Newer_Cameron <- sbchoice(
  CV ~ 
    (LogBidIncome * Gender) +
    AgeDummy +
    EthnicityDummy +
    MeanExpectedFuture * Q16_MicroplasticsCurrentSelf +
    Variance + 
    Charity +
    Consequentiality +
    Order +
...
SBDC_Model_Newer_Cameron %>% summary()
# *****************************
# Section 5: Export Model ####
# *****************************


SBDC_Model_Newer_Future %>% saveRDS(
  here("CVoutput","CVModel_SBDC_Model_Newer_Future.rds")
)



# *****************************
# Section 6: Model-specific WTP ####
# *****************************


ModelWTP <- SBDC_Model_Newer_Future %>% bootCI()

paste0(
  "we calculate the median WTP as £",
  ModelWTP$out[4, 1] %>% round(2),
  " (£", 
  ModelWTP$out[4, 2] %>% round(2),
  " - £",
  ModelWTP$out[4, 3] %>% round(2),
