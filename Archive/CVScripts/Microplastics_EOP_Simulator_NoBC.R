...
EOP_Maker <- function(Data, Means, Variances, Y) {
  ...
  Delta_0 <- stage_2_Simple$coefficients[2] %>% as.numeric()
  # Delta_0 <- 1.850919
  Delta_1 <- stage_2_Simple$coefficients[3] %>% as.numeric() 
  # Delta_1 <- 15.58868
...
Means <-
  c(I((
    predict(stage_1_Simple, type = "response") + Data$MEC
  ) / 2))
...
SummaryTable %>% 
  data.frame() %>% 
  fwrite(sep = ",",
                here("Data", "EOP_SummaryTable_2211.txt"))
## formally _1710 version which didn't have % income


## Correlation test here
# cor.test(c(I((predict(stage_1_Simple, type = "response") + Data$MEC)/2)), (0 - (betareg::predict(stage_1_Simple,type = "variance"))))
