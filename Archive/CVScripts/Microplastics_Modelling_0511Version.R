...
## Store all misc changes here
Data %>% 
  data.frame() %>% 
  fwrite(sep = ",",
         here("Data", "Data_Updated_24_11_27.csv"))
...
Simulator <- function(data, 
  ...
  boot.function <- function(data, indices) {
    ...
    fit_stats <- c(
      ...
      s2_PseudoR2 = (1 - stage_2$deviance/stage_2$nulldev)   # McFadden's R2
    )
  ...
  l <- length(boot.results$t0) - 6  # Subtract 3 for the fit statistics
  ...
  fit_stats_means <- c(
    S1_AIC = mean(boot.results$t[, l + 1]),
    S1_LogLik = mean(boot.results$t[, l + 2]),
    S1_PseudoR2 = mean(boot.results$t[, l + 3]),
    S2_AIC = mean(boot.results$t[, l + 4]),
    S2_LogLik = mean(boot.results$t[, l + 5]),
    S2_PseudoR2 = mean(boot.results$t[, l + 6])
  ) %>% round(3)
...
summary_function <- function(EOP) {
  cbind(paste0(round(mean(EOP, na.rm = TRUE), 3), 
               " (",
               round(sd(EOP, na.rm = TRUE), 3),
               ")"))
}
...
# Call the simulator function
Model1_EOP <- EOP_Maker(data = Data,
                        formula_stage_1 = Model1_stage1_formula,
                        formula_stage_2 = Model1_stage2_formula,
                        R = 1000
)  
