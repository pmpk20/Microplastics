...
# ********************************************
# S4: B1 All used covariates to dispersion ####
# ********************************************
...
# Define your formula for stage_1 and stage_2 models
formula_stage_2_B1 <- "-1 + LogBidIncome"

# Call the simulator function
Model_B1 <- Simulator(data = Data,
                      formula_stage_1 = formula_stage_1_B1,
                      formula_stage_2 = formula_stage_2_B1,
                      R = R
)  


Model_B1_Diagnostics <- cbind(
  "Variable" = Model_B1$fit_statistics %>% names(),
  "Estimate" = Model_B1$fit_statistics
) %>% data.frame()


Model_B1_Output <- rbind(
  Model_B1$coefficients %>% 
    ModelOutput(Identifier = 1) %>% 
    dplyr::select(-Model),
  Model_B1_Diagnostics
) 

Model_B1_Output %>% write.csv(quote = FALSE, row.names = FALSE)


Model_B1_Output %>% 
  data.frame() %>% 
  fwrite(sep = ",",
         here("Tables", "Table_Model_B1_Output.txt"))



# ********************************************
# S4: B2 All possible covariates to dispersion ####
# ********************************************
...
# Define your formula for stage_1 and stage_2 models
formula_stage_2_B2 <- "-1 + LogBidIncome"

# Call the simulator function
Model_B2 <- Simulator(data = Data,
                      formula_stage_1 = formula_stage_1_B2,
                      formula_stage_2 = formula_stage_2_B2,
                      R = R
)  


Model_B2_Diagnostics <- cbind(
  "Variable" = Model_B2$fit_statistics %>% names(),
  "Estimate" = Model_B2$fit_statistics
) %>% data.frame()


Model_B2_Output <- rbind(
  Model_B2$coefficients %>% 
    ModelOutput(Identifier = 1) %>% 
    dplyr::select(-Model),
  Model_B2_Diagnostics
) 

Model_B2_Output %>% write.csv(quote = FALSE, row.names = FALSE)


Model_B2_Output %>% 
  data.frame() %>% 
  fwrite(sep = ",",
         here("Tables", "Table_Model_B2_Output.txt"))
...
# Define your formula for stage_1 and stage_2 models
formula_stage_2_B3 <- "-1 + LogBidIncome"

# Call the simulator function
Model_B3 <- Simulator(data = Data,
                      formula_stage_1 = formula_stage_1_B3,
                      formula_stage_2 = formula_stage_2_B3,
                      R = R
)  


Model_B3_Diagnostics <- cbind(
  "Variable" = Model_B3$fit_statistics %>% names(),
  "Estimate" = Model_B3$fit_statistics
) %>% data.frame()


Model_B3_Output <- rbind(
  Model_B3$coefficients %>% 
    ModelOutput(Identifier = 1) %>% 
    dplyr::select(-Model),
  Model_B3_Diagnostics
) 

Model_B3_Output %>% write.csv(quote = FALSE, row.names = FALSE)


Model_B3_Output %>% 
  data.frame()  %>% 
  fwrite(sep = ",",
         here("Tables", "Table_Model_B3_Output.txt"))
...
# Define your formula for stage_1 and stage_2 models
formula_stage_2_B4 <- "-1 + LogBidIncome"

# Call the simulator function
Model_B4 <- Simulator(data = Data,
                      formula_stage_1 = formula_stage_1_B4,
                      formula_stage_2 = formula_stage_2_B4,
                      R = R
)  


Model_B4_Diagnostics <- cbind(
  "Variable" = Model_B4$fit_statistics %>% names(),
  "Estimate" = Model_B4$fit_statistics
) %>% data.frame()


Model_B4_Output <- rbind(
  Model_B4$coefficients %>% 
    ModelOutput(Identifier = 1) %>% 
    dplyr::select(-Model),
  Model_B4_Diagnostics
) 

Model_B4_Output %>% write.csv(quote = FALSE, row.names = FALSE)
