...
# ****************************************
# Section 2: Cameron's adhoc approach ####
# ****************************************


## Specify basic Cameron model here
mod.cameron <-
  glm(
    CV ~ -1 + ## no intercept
      LogBidIncome + ## bid vector
      Consequentiality + 
      Charity + ## Two very significant covariates but not as interaction terms
      I((MEF + MEC) / 2) + ## means
      I(-var.cameron), ## variances
...
## Explore estimates
# all significant
# LBI positive, mean positive, variance negative 
mod.cameron %>% summary()


## Test model coefficients
coeftest(mod.cameron)

# *************************************************
# Section 3: Naive 2stage GLM, inconsistent SE ####
# *************************************************


## Estimate predictors of mean expectations
## let's later add more covariates here
stage_1 <-
  betareg(MEF ~ 
            -1 + 
            as.factor(AgeBracket) + 
            as.factor(Education) + 
            as.factor(Ethnicity) +
          ...
          Data, 
          type = "BC") ## bias corrected measure


coeftest(stage_1)


## Very basic model where CV = no intercept, LBI, mean, variance
## using 0 - predict to enforce a negative
## predict.betareg to be precise
stage_2 <- glm(CV ~ -1 + 
                 LogBidIncome + 
                 I((betareg::predict(stage_1, type = "response") + MEC) / 2) + 
                 I(0 - betareg::predict(stage_1, type = "variance")), 
               family = binomial(link = "probit"), 
               data = Data)
...
# ********************************************
# Section 4: Proper 2stage, consistent SE ####
# ********************************************
...
boot.function <- function(data, indices){
  ...
  return(c(summary(stage_1)$coefficients$mean[ , c(1) ], 
           summary(stage_1)$coefficients$precision[ , c(1) ],
           summary(stage_2)$coefficients[ ,c(1) ],
           summary(stage_1)$coefficients$mean[ , c(2) ], 
           summary(stage_1)$coefficients$precision[ , c(2) ],
           summary(stage_2)$coefficients[ ,c(2) ]))
}
...
#compare to naive SE
summary(stage_1)$coefficients$mean[,c(1:2)]
summary(stage_1)$coefficients$precision[,c(1:2)]
summary(stage_2)$coefficients[,c(1:2)]



# LeftMost <- rbind(
#   summary(stage_1)$coefficients$mean %>% round(3),
#   summary(stage_1)$coefficients$precision %>% round(3),
#   summary(stage_2)$coefficients %>% round(3)
# )
# 
# 
# ## Output to csv for word doc
# cbind(LeftMost , results_rounded_withP) %>% 
#   write.csv(quote = FALSE)

# *************************************************
# Section 5: Expected Option Prices ####
# *************************************************
...
Delta_0 <- results['I((predict(stage_1, type = "response") + MEC)/2)', "Estimate"] %>% as.numeric()
Delta_1 <- results['I(0 - predict(stage_1, type = "variance"))', "Estimate"] %>% as.numeric() 


A <-
  ((Delta_0*c(Data$NewMean)) +
     (Delta_1 * mean(betareg::predict(stage_1,type = "variance"))
  )) %>% as.numeric()
