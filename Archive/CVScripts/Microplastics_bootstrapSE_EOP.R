...
## - Changed EOP::delta_1 to multiply mean(predictions) not Data$Uncertainty
...
stage_2_Simple <- glm(CV ~ -1 +
                 LogBidIncome + 
                 I((predict(stage_1_Simple,type="response")+MEC)/2) + 
...
#compare to naive SE
summary(stage_1_Simple)$coefficients$mean[,c(1:2)]
summary(stage_1_Simple)$coefficients$precision[,c(1:2)]
summary(stage_2_Simple)$coefficients[,c(1:2)]
...
A <-
  ((Delta_0 * c(I((predict(stage_1_Simple, type = "response") + Data$MEC)/2)) +
      (Delta_1 * (0 - mean(betareg::predict(stage_1_Simple,type = "variance"))))
  )) %>% as.numeric()
...
PlotData <- bind_cols(
  "Means" = I((predict(stage_1_Simple,type="response")+Data$MEC)/2),
  "Variances" = I(0- predict(stage_1_Simple, type = "variance")),
  "Income" = Data$Income_Annual_Quintile,
  "EOP" = findInterval(Data$EOP, 
                       quantile(Data$EOP, 
                                c(0.025, 0.25, 0.5, 0.75, 0.975)),rightmost.closed = TRUE) %>% as.factor() 
) 
