...
## Repeats LM tests
# WTPsimulator <- function(Model, Data) {
# 
#   
#   
#   for (i in 1:nrow(Data)){
#     MeanLoop_Var1[[i]] <- DCchoice::krCI(SBDC_Model_3,
...
#     
#   }
#   TableData_Var1 = do.call(rbind,  MeanLoop_Var1) %>% data.frame()
#   
#   
#   
...
WTP_Tester <- function(Input) {
  WTP <- matrix(data = 0, nrow = nrow(Data), ncol = 1) %>% as.data.frame()
  WTP <-
    foreach(
      i = 1:nrow(Data),
      .combine = c,
      .export = c("WTPsimulator", "Data", "SBDC_Model_3"),
      .packages = list.of.packages
    ) %dopar% {
      
      
      DCchoice::krCI(SBDC_Model_3,
                     individual = data.frame(MeanExpectedFuture = Data$MeanExpectedFuture[i],
                                             Mean_Change = Data$Mean_Change[i],
                                             Mean_Square = Data$Mean_Square[i],
                                             Variance = Input,
                                             ...
                                             AgeDummy = Data$AgeDummy[i],
                                             Consequentiality = Data$Consequentiality[i],
                                             Order = Data$Order[i]))$out[1,1]  %>% as.numeric()
    }
  WTP %>% as.data.frame()
}
WTP_Tester(1)
...
# ***************************************************
# Section X: Recovering WTP: Variance 1 ####
# ***************************************************


MeanLoop_Var1 = vector("list", length = (Data %>% nrow()))
...
# ***************************************************
# Section X: Recovering WTP: Variance 2 ####
# ***************************************************


MeanLoop_Var2 = vector("list", length = (Data %>% nrow()))

for (i in 1:nrow(Data)){
  MeanLoop_Var2[[i]] <- DCchoice::krCI(SBDC_Model_3,
                                       individual = data.frame(MeanExpectedFuture = Data$MeanExpectedFuture[i],
                                                               Mean_Change = Data$Mean_Change[i],
                                                               Mean_Square = Data$Mean_Square[i],
                                                               Variance = 4,
...
TableData_Var2 = do.call(rbind,  MeanLoop_Var2) %>% data.frame()
