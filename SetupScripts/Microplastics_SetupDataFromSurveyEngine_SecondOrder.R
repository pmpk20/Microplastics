#### DEFRA: Microplastics ####
## Function: Imports and transforms the second bratch from prolific
...
Covariates <- here("Data/Raw data", "kent_eapmicro_liveordercecv_covariates.xlsx") %>% read_excel() %>% data.frame()
DCE <- here("Data/Raw data", "kent_eapmicro_liveordercecv_dce.xlsx") %>% read_excel() %>% data.frame()
Timers <- here("Data/Raw data", "KENT_EAPMICRO_LiveOrderCECV_pages.xlsx") %>% read_excel() %>% data.frame()
...
## Recoding Gender:
Combined_Trimmed$q2 <- dplyr::recode(Combined_Trimmed$q2,'1'=0,'2'=1,"3"=2,"4"=3)
...
Combined_Trimmed$q8 <- dplyr::recode(Combined_Trimmed$q8,
                              '1'=0,
                              '2'=100,
                              "3"=200,
                              "4"=300,
...
### dplyr::recoded for Yes = 1, No = 0
Combined_Trimmed$CV <- coalesce(Combined_Trimmed$q9,Combined_Trimmed$q10)
Combined_Trimmed$CV <- dplyr::recode(Combined_Trimmed$CV,'1'=1,'2'=0)
...
Combined_Trimmed$Coronavirus %<>% dplyr::recode('1'=1,'2'=0)
## Recoding so had it ==1, no ==0

colnames(Combined_Trimmed)[which(names(Combined_Trimmed)=="q19")] <- "Charity"
Combined_Trimmed$Charity %<>% dplyr::recode('1'=1,'2'=0)
...
Combined_Trimmed$Consequentiality %<>% dplyr::recode('1'=1,'2'=0)
...
Combined_Trimmed$Income <- dplyr::recode(Combined_Trimmed$Income,
                                  "1"=250,
                                  "2" = 750,
                                  "3"=1250,
                                  "4"=1750,
...
## dplyr::recode NA or missing as ZERO
...
## dplyr::recode Attribute Levels:
database$Price_A <- dplyr::recode(database$Price_A,'1'=0.00)
database$Price_B <- dplyr::recode(database$Price_B,'1'=0.50,'2'=1.00,"3"=2.50,"4"=5.00)
database$Price_C <- dplyr::recode(database$Price_C,'1'=0.50,'2'=1.00,"3"=2.50,"4"=5.00)

database$Performance_A <- dplyr::recode(database$Performance_A,'1'=0.00)
database$Performance_B <- dplyr::recode(database$Performance_B,'1'=5,'2'=10,"3"=50)
database$Performance_C <- dplyr::recode(database$Performance_C,'1'=5,'2'=10,"3"=50)


database$Emission_A <- dplyr::recode(database$Emission_A,'1'=0.00)
database$Emission_B <- dplyr::recode(database$Emission_B,'1'=10,'2'=40,"3"=90)
database$Emission_C <- dplyr::recode(database$Emission_C,'1'=10,'2'=40,"3"=90)


database$Order <- 1
Combined_Trimmed$Order <- 1
...
write.csv(Combined_Trimmed,file = paste0("Microplastics_Order1_Long_",Date,".csv"))
write.csv(database,file = paste0("Microplastics_Order1_Wide_",Date,".csv"))



# ----------------------------------------------------------------------------------------------------------
## Exporting anonymised data:
### This can only be done after estimating WTP

database <- data.frame(read.csv("Microplastics_Order1_Wide_2022_06_18.csv"))
Data <- data.frame(read.csv("Microplastics_SEOrder1_DataWithAllWTP_2022_06_18.csv"))

## Order 1: CE then CV questions 
Data$Order <- 1
...
database_Anonymised <- database[, -which(
  names(Data) %in% c("VERSION","IP",
                     "X","X.2","Postcode",
                     "X.1",
                     "X.3",
...
write.csv(Data_Anonymised,file = paste0("Microplastics_Order1_LongFormat_Anonymised_",Date,".csv"))
write.csv(database_Anonymised,file = paste0("Microplastics_Order1_WideFormat_Anonymised_",Date,".csv"))
