...
SecondPilot25 <- data.frame(read.csv("SecondPilot25_Transformed_2022_05_10.csv"))
...
Attitudes_Melted %>% ggplot(aes(x = value, y = variable, group=variable,fill = variable)) + 
  ...
  scale_x_continuous(name = "Expected Harm on a scale -10 to +10.",
                     limits = c(-10, 10),
                     breaks = seq(-10, 10, 2)) +scale_y_continuous(
...
# Join Data: #------------------------------
colnames(SecondPilot25)[which(names(SecondPilot25)=="X")] <- "ID"
SecondPilot25_Spatial <- as.data.frame(SecondPilot25) %>% 
  st_as_sf(coords=c("Longitude","Latitude"), crs=4326, remove=FALSE)  

GB_Pilot <- st_join(GB, left = FALSE, SecondPilot25_Spatial)
# GB_Pilot <- st_join(SecondPilot25_Spatial, left = FALSE, GB) # join points
