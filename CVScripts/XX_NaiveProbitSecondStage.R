...
stage_2_summary <- summary(stage_2)
coef_table <- stage_2_summary$coefficients
...
diagnostics <- data.frame(
  Variable = c("AIC", "Log-likelihood", "Pseudo-R²", "EOP (SD)"),
  Coefficient = c(sprintf("%.3f", fit_stats[["s2_AIC"]]),
                  sprintf("%.3f", fit_stats[["s2_LogLik"]]),
                  sprintf("%.3f", fit_stats[["s2_PseudoR2"]]),
...
final_table %>%
  ...
  fwrite(
    ...
    here("CVoutput/Tables", 
         paste0("Table_X_NaiveProbitSecondStage.txt"))
  )
